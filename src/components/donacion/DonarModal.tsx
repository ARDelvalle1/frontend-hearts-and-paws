      ...
      <div className="relative bg-white dark:bg-[#28180d] rounded-3xl shadow-2xl max-w-md w-full p-8 border border-[#6c2f00]/15 dark:border-[#ffdbc9]/15 overflow-hidden text-[#28180d] dark:text-[#ffede4]">
        ...
        {loading && (
          <div className="absolute inset-0 z-30 bg-white/95 dark:bg-[#28180d]/95 backdrop-blur-xs flex flex-col items-center justify-center text-center p-6">
            ...
            <p className="text-xs text-[#54433a] dark:text-[#dac2b6] mt-2">
              Tu donación será procesada de forma segura. No cierres ni recargues esta ventana.
            </p>
          </div>
        )}

        <h2 className="font-display-editorial text-2xl font-bold text-[#6c2f00] dark:text-[#ffdbc9] text-center mb-2 flex items-center justify-center gap-2">
          <span className="material-symbols-outlined text-2xl text-[#c85a32]">favorite</span>
          Doná para ayudar
        </h2>
        <p className="text-center mb-6 text-[#54433a] dark:text-[#dac2b6] text-sm">
          Faltan <strong className="text-[#6c2f00] dark:text-[#ffdbc9]">${faltante.toLocaleString('es-AR')}</strong> para alcanzar la meta.
        </p>
        ...
        <div className="mb-6">
          ...
          />
          {errorMonto && (
            <p className="text-xs text-red-600 dark:text-red-400 mt-1.5 text-center font-semibold">{errorMonto}</p>
          )}
        </div>
